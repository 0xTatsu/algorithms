...
# print(MaxSumSubArray([1, 4, 2, 10, 2, 3, 1, 0, 20], 4) == 24)
    ...
    for right_window, item in enumerate(s):
        if item in hashTable:
            if hashTable[item] > left_window:  # abbbbba
                left_window = hashTable[item] + 1 # if duplicated, set left to the first appeared + 1
...
#         while count[current_char] > 1: # why WHILE: abcded
